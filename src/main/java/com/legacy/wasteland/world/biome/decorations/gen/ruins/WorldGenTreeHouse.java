...
public class WorldGenTreeHouse extends WorldGenerator {
   public boolean generate(World world, Random rand, BlockPos pos) {
      if(world.getBlockState(pos.down()).getBlock() == WorldConfig.getSurfaceBlock()) {
         ...
         String[] usedLootTable = rand.nextInt(3) == 0?LootConfig.ruinRareLoot:LootConfig.ruinEasyLoot;

         for(int treasureSize = 0; treasureSize < 2 + rand.nextInt(2); ++treasureSize) {
            chest1.setInventorySlotContents(rand.nextInt(chest1.getSizeInventory()), LootConfig.getLoot(usedLootTable)[rand.nextInt(usedLootTable.length)]);
         }
